...
const service = {
  ...
  async registerUser(req, res) {
    try {
      ...
      <a href="https://priceless-boyd-d4fac3.netlify.app/verifyuser/${insertedId}">Click here</a>`;
  ...
  async loginUser(req, res) {
    ...
    if (!user.isVerified) {
      const link = `<p>Hi user, welcome to Dietify</p>
         <p>Kindly click the link below to verify</p><br/>
       <a href="https://priceless-boyd-d4fac3.netlify.app/verifyuser/${user._id}">Click here</a>`;
      await sendMail(value.email, "Verify User", link);
  ...
  async sendPasswordResetLink(req, res) {
    ...
    const link = `<p>Hi user, password Reset verification from Dietify</p>
       ...
       <a href="https://priceless-boyd-d4fac3.netlify.app/passverifylink/${user._id}/${token}">Click here</a>`;
